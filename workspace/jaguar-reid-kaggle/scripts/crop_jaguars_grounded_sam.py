            ...
            if sam_processor is not None and sam_model is not None:
                try:
                    ...
                    if masks.numel() > 0:
                        ...
                        m = np.clip(m, 0.0, 1.0)
                        crop = contour_crop_from_mask(image, m, args.padding, crisp_contour=args.crisp_contour, crisp_threshold=args.crisp_threshold, exclude_green=args.exclude_green)
                        if crop is not None:
                            label_slug = "jaguar"
